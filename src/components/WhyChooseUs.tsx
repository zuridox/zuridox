...
const reasons = [
  {
    ...
    description: "Over a decade of experience delivering cutting-edge IT solutions across diverse industries."
  },
  {
    icon: Clock,
    title: "Rapid Delivery",
    description: "Agile methodology ensuring quick turnaround times without compromising quality."
  },
  {
    icon: Target,
  ...
  },
  {
    ...
    description: "Multiple awards and certifications recognizing our excellence in IT consulting."
  }
];
...
export default function WhyChooseUs() {
  ...
  return (
    <section className={`py-24 ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className={`text-4xl font-extrabold mb-6 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
            Why Choose Us
          </h2>
          <p className={`text-xl ${theme === 'dark' ? 'text-gray-400' : 'text-gray-700'} max-w-2xl mx-auto`}>
            Partner with us for innovative solutions that drive real business growth.
          </p>
        </div>
          ...
          {reasons.map((reason, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Icon
                    className={`w-10 h-10 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`} // Adjusting icon color based on theme
                  />
                </div>
                ...
                <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-800'}`}>{reason.description}</p>
              </div>
            );
          })}
        </div>
